...
import { getUserByEmail, formatArrayForPostgres } from '@/lib/api/auth.js';
...
async function handler(req, res) {
  ...
  if (existingUser) {
    ...
    if (Array.isArray(activeSubs) && activeSubs.length > 0 && data.planType === 'professional') {
      // Already subscribed to this plan — let the client send them to their
      // dashboard gracefully instead of hard-failing with a 409.
      return res.json({
        success: true, existingUser: true, hasActiveSubscription: true,
        userId: existingUser.id, userEmail: existingUser.email,
        message: 'You already have the Professional plan.'
      });
    }
